...
app.get("/get-tours", async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: "An error occurred while fetching tours" });
    }
});
...
app.post("/mark-tour-complete", async (req, res) => {
    ...
    try {
        ...
        const tour = await client
            .db("Tours")
            .collection("Applications")
            .findOne({ _id: new ObjectId(tourId), "guide.id": new ObjectId(guideId) });

        if (!tour) {
            return res.status(404).json({ error: "Tour not found or not assigned to this guide." });
        }

        // Mark the tour as completed in the Applications collection
        ...
        if (updateResult.matchedCount === 0) {
            return res.status(500).json({ error: "Failed to mark the tour as completed." });
        }

        // Remove the `_id` field from the copied object to avoid conflicts
        const { _id, ...tourWithoutId } = tour;

        // Add the completed timestamp and updated status
        const completedTour = {
            ...tourWithoutId,
            status: "Completed",
        ...
        // Insert the completed tour into the CompletedTours collection
        const insertResult = await client
            .db("Tours")
            .collection("CompletedTours")
            .insertOne(completedTour);

        if (!insertResult.insertedId) {
            return res.status(500).json({ error: "Failed to save completed tour." });
        }
...
app.get("/get-completed-tours", async (req, res) => {
    try {
        const completedTours = await client
            .db("Tours")
            .collection("CompletedTours")
            .find({})
            .toArray();

        return res.status(200).json(completedTours);
    } catch (error) {
        console.error("Error fetching completed tours:", error);
        return res.status(500).json({ error: "An error occurred while fetching completed tours." });
    }
});
